...
fn display_open_elections() -> Result<(String, String), Box<dyn std::error::Error>> {
    ...
    println!(
        "You have selected: {}: {}",
        selected_election.election_id, selected_election.name
    );

    Ok((
        selected_election.election_id.clone(),
        selected_election.name.clone(),
    ))
}
...
fn display_candidates(
    ...
    for result in rdr.deserialize() {
        ...
        if office_ids.contains(&office.office_id) {
            offices.insert(office.office_id.clone(), office.office_name.clone());
        }
    }
    ...
    for candidate in candidates {
        candidates_by_office
            .entry(candidate.office_id.clone())
...
fn cast_vote(
    ...
    info!(
        "Casting vote process started for user {} in election {}",
        user_id, election_id
    );
    ...
    info!(
        "Vote successfully cast for user {} in election {}, who voted for: {}",
...
fn change_voter_status(user_id: String) -> Result<(), Box<dyn Error>> {
    ...
    info!("Voter status changed for user {} at {}", user_id, timestamp);
...
pub fn authenticate_voter() -> Result<(), Box<dyn StdError>> {
    ...
    for result in rdr.deserialize() {
        ...
        if record.national_id == national_id && record.date_of_birth == date_of_birth {
            ...
            if record.has_voted {
                println!(
                    "Our records indicate you have already voted. Thank you for participating!"
                );
    ...
    if !user_found {
        ...
        return Err(
            "User not found or unregistered. Please check your national ID and date of birth."
                .into(),
        );
    }
