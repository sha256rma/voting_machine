...
pub fn tally_vote() -> Result<(), Box<dyn Error>> {
    ...
    info!("Loaded {} elections", elections.len());
    debug!("Elections: {:?}", elections);

    let mut candidates_rdr = ReaderBuilder::new()
        .trim(Trim::All)
        .from_path("candidates.csv")?;
    let candidates: Vec<Candidate> = candidates_rdr
        .deserialize()
    ...
    let mut offices_rdr = ReaderBuilder::new()
        .trim(Trim::All)
        .from_path("offices.csv")?;
    let offices: Vec<Office> = offices_rdr.deserialize().filter_map(Result::ok).collect();
    info!("Loaded {} offices", offices.len());
    debug!("Offices: {:?}", offices);
    ...
    if let Some(election) = elections
        .iter()
        .find(|e| e.election_id == election_id_input)
    {
        if election.is_open {
...
fn tally_votes_for_election(
    election_id: &String,
    candidates: &Vec<Candidate>,
    offices: &Vec<Office>,
) {
    let mut vote_rdr = ReaderBuilder::new()
        .trim(Trim::All)
        .from_path("votes.csv")
        .expect("Unable to open votes.csv");
    let votes: Vec<Vote> = vote_rdr.deserialize().filter_map(Result::ok).collect();
    info!("Loaded {} votes", votes.len());
    debug!("Votes: {:?}", votes);
    ...
    for office in offices.iter().filter(|o| o.election_id == *election_id) {
        ...
        for vote in votes.iter().filter(|v| v.election_id == *election_id) {
            if let Some(candidate) = candidates
                .iter()
                .find(|c| c.candidate_id == vote.candidate_id && c.office_id == office.office_id)
            {
                *tally.entry(candidate.candidate_id.clone()).or_insert(0) += 1;
            }
        }

        if let Some((winner_id, total_votes)) = tally.iter().max_by_key(|entry| entry.1) {
            if let Some(winner_candidate) = candidates.iter().find(|c| c.candidate_id == *winner_id)
            {
                println!(
                    "Office: {}, Winner: {}, Party: {}, Total Votes: {}",
                    office.office_name,
                    winner_candidate.name,
                    winner_candidate.political_party,
                    total_votes
                );
            } else {
                println!(
                    "Candidate details not found for the winner of office {}.",
                    office.office_name
                );
            }
        } else {
            println!(
                "No votes found for office {} in this election.",
                office.office_name
            );
        }
    }
